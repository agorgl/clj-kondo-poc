...
* [`src/clj_kondo/poc/styles.cljs`](src/clj_kondo/poc/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/clj_kondo/poc/`](src/clj_kondo/poc/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/clj_kondo/poc/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `clj-kondo.poc` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/clj_kondo/poc/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns clj-kondo.poc.example
  (:require [clj-kondo.poc.config :as config])
